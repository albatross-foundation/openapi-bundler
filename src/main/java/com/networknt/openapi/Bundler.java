...
public class Bundler {
    ...
    @Parameter(names = {"--merge", "-m"},
            description = "Merge multiple yaml file")
    String merge;
    ...
    public void run(JCommander jCommander, String... argv) {
        ...
        if (operation.equalsIgnoreCase("validate")) {
            ...
            Utils.validateSpecification(Paths.get(
                    dir,
                    fileName));
            return;
        }
        ...
        try {
            if ("merge".equals(merge)){
                MergeProcessor mergeProcessor = new MergeProcessor(folder, fileName, outputDir, outputFile);
                mergeProcessor.merge(dir);
            } else {
